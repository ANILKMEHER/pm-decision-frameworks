# Executive Decision-Making Frameworks Playbook 🚀

Welcome to the **Executive Decision-Making Frameworks Playbook**. This repository serves as a practical, structured guide to the world's most effective analytical methods, prioritization funnels, and problem-solving mental models used in Technical Product Management, business strategy, and engineering leadership.

The goal of this toolkit is to move from gut-feeling decisions to structured, data-driven, and defensible frameworks.

---

## 📋 Table of Contents & The 10 Core Frameworks

### 🏛️ Section 1: Strategic & Environmental Analysis
1. **[SWOT Analysis](./strategic-analysis/SWOT-analysis.md)** (Strengths, Weaknesses, Opportunities, Threats)
   * *When to use:* Evaluating internal capabilities vs. external market realities.
2. **PESTLE Analysis** (Political, Economic, Social, Technological, Legal, Environmental)
   * *When to use:* High-level macro-environmental analysis for long-term product roadmaps.

### 🔍 Section 2: Root Cause Analysis & Problem Solving
3. **[Fishbone (Ishikawa) Diagram](./root-cause-problem-solving/fishbone-ishikawa.md)**
   * *When to use:* Visualizing and categorizing potential causes of a complex systemic problem or failure.
4. **The 5 Whys Methodology**
   * *When to use:* Drilling down rapidly to the root cause of an incident, bug, or operational bottleneck.

### 🎯 Section 3: Prioritization Funnels & Frameworks
5. **RICE Scoring Model** (Reach, Impact, Confidence, Effort)
   * *When to use:* Objective backlog grooming and feature prioritization.
6. **MoSCoW Method** (Must have, Should have, Could have, Won't have)
   * *When to use:* Setting immediate MVP boundaries and release scoping with stakeholders.
7. **WSJF** (Weighted Shortest Job First)
   * *When to use:* Prioritizing complex features in Agile/SAFe environments by calculating the cost of delay.

### ⚡ Section 4: Advanced Hierarchy & Decision-Making
8. **AHP (Analytic Hierarchy Process)**
   * *When to use:* Multi-criteria decision-making when selecting vendors, architectures, or competing high-stakes strategies.
9. **The Cynefin Framework**
   * *When to use:* Categorizing contexts (Obvious, Complicated, Complex, Chaotic) to choose the right management response.
10. **RACI Matrix** (Responsible, Accountable, Consulted, Informed)
    * *When to use:* Establishing cross-functional governance, ownership, and communication streams for execution.

---

## 🚀 How to Use This Playbook
1. **Browse by Problem Type:** Navigate to the specific folder corresponding to your current challenge (Strategic, Prioritization, Root Cause).
2. **Apply the Markdown Templates:** Copy the ready-made templates into your own project management tools, documentation spaces, or wiki pages.
3. **Contribute:** Feel free to open a Pull Request if you have a real-world case study or an additional framework to add!
